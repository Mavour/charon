...
export async function decideCandidateBatch(rows, triggerCandidateId) {
  ...
  const system = [
    ...
    '',
    'Goal: find tokens with momentum. In dry-run mode we prioritize volume and learning over extreme selectivity.',
    '',
    'DECISION STRUCTURE:',
    'Confidence score 0-100. Thresholds:',
    '- Confidence >= 35 -> BUY (dry-run tolerance, learn from many trades)',
    '- 20-34 -> WATCH',
    '- <20 -> PASS',
    '',
    ...
    '   - Market cap $35k-$100k -> 0 (neutral, still tradeable)',
    '   - Market cap > $100k -> -10 (higher cap, lower upside)',
    '   - Market cap < $5k -> -5 (small, but acceptable for microcap)',
